...
\begin{enumerate}
    \item \textbf{Artefact} – representing an individual artefact and containing all attributes from the \texttt{artefact} table described in Chapter \ref{cha:system_design}, Section \ref{sec:database}.  
    \item \textbf{Dimensions} – a substructure of \textbf{Artefact}, storing three measurements: weight, height, and width.  
    \item \textbf{Artefact List} – a collection of \textbf{Artefact} structures to manage multiple objects retrieved from the database.  
\end{enumerate}
...
The dropdown options retrieval mechanism is responsible for retrieving the \glspl{ID} of all available artefacts from the database.
These artefacts are stored in a list structure, which is then iterated to populate the dropdown \texttt{options} list, as illustrated in Listing~\ref{lst:artifact_ids}.
    ...
    IEnumerator GetOptionsIds(string url)
{
    ...
    {
        ArtefactList artefactList = JsonUtility.FromJson<ArtefactList>(wrappedJson);
        
        List<TMP_Dropdown.OptionData> options = new List<TMP_Dropdown.OptionData>();
        ...
        // Iterate through all artefacts and add their IDs to options List
        foreach (Artefact artefact in artefactList.items)
        {
            string id = artefact.id.ToString();
...
Additionally, textual data is retrieved. Once the request is successful, the JSON response is parsed into an \texttt{Artefact} object using Unity's \texttt{JsonUtility.FromJson<Artefact>()} method.  
...
{   
    if (www.result == UnityWebRequest.Result.Success)
    {
        artefact = JsonUtility.FromJson<Artefact>(www.downloadHandler.text);

        string displayText = "<b>Object Details:</b>\n\n";
        // Build the final text with detailed artefact information
            displayText += $"ID: {artefact.id} | Name: {artefact.name} \nMaterial: {artefact.material} | Epoch: {artefact.epoch} \nProvenance: {artefact.provenance}" +
                        $"\nDimensions: {artefact.dimensions.height} x {artefact.dimensions.width} cm, Weight: {Mathf.Round(artefact.dimensions.weight * 10.0f) * 0.1f} g";
        // Update the UI Panel with the artefact text details
        resultText.text = displayText;
        // Call the function to load images before intervention of the artefact
        GetImages(artefact.id, artefact.images_path_b_interv);
    }
    ...
}
\end{lstlisting}
...
With these materials, a "Combined" texture was baked and imported to Unity as a .png file.
 ...
 \begin{figure}[h!]
    \centering
    \includegraphics[width=0.76\linewidth]{Implementation/Glass_texture}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{model3}
