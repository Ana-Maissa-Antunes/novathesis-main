...
In this thesis, tangible objects were digitally reproduced and represented as \gls{3D} models.
...
A potential application of the \gls{DT} technology for heritage is its accurate
...
\section{Interaction and Visualization Technologies}
...
that can be grabbed and moved (or grant the grab and move ability to another object), constraint maintainers 
...
Common \gls{VE} input devices include \gls{6-DOF} trackers\footnote{\url{https://www.ar.rocks/glossary/6dof-tracking}} as exemplified in Figure \ref{fig:input_device}, gloves capable of recognizing continuous postures or discrete events, pen-like, simple button devices, as well as specialized tools such as the Spaceball or joysticks with force-feedback.

The following paragraphs describe four common available \gls{VE} task categories.

One fundamental interaction task is selection, which consists of specifying an object or set of objects for a particular action. 
...
Another important task is manipulation, understood as the modification of an object’s attributes, such as its identifier, name, position, scale, or texture. These tasks are particularly significant in fields like design, prototyping, simulation, and entertainment, where environments frequently need to be adjusted by the user.
...
A third major interaction task is travel, also called viewpoint motion control, and is the most ubiquitous \gls{VE} interaction 
...
In addition to these tasks, many other interactions found in \gls{VE} applications fall under the heading of system control.
...
The three common \gls{VE} display devices are Head-Mounted Displays (\glspl{HMD}), as illustrated in Figure \ref{fig:HMDs}, \glspl{SID} (fully or 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.55\linewidth]{HMD1}
    \caption{\gls{CAVE} environment with multiple stereoscopic projection surfaces~\cite{SHERMAN2019258}.} 
    \label{fig:HMD1}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.35\linewidth]{display}
...
\subsection{Artefacts Reconstruction}
